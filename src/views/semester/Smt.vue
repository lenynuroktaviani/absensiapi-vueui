<template>
  ...
  <tbody>
    <tr v-for="(semesters, index) in semester" :key="index">
      ...
      <td>
        <router-link class="btn btn-success" :to="{name:'Editsmt', params:{id:semesters.id}}">Edit</router-link>
        <button @click.prevent="smtDelete(semesters.id)" class="btn btn-danger">Delete</button>
      </td>
    </tr>
  </tbody>
</table>
